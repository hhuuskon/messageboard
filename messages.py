...
def get_messages(id):
    sql = "SELECT DISTINCT U.username, M.content, M.sent_at, T.id, T.subjects, M.id FROM messages M, topics T, users U " \
        "WHERE M.topic_id=T.id AND T.id=:id AND M.user_id=U.id ORDER BY M.sent_at DESC"
...
def new_submessage(message, visibility, message_id):
    ...
    if user_id == 0:
        return False
    sql = "INSERT INTO submessages (message_id, content, user_id, visibility) VALUES (:message_id, :content, :user_id, :visibility)"
    db.session.execute(sql, {"message_id":message_id, "content":message, "user_id":user_id, "visibility":visibility})
    db.session.commit()
    return True

def search_own_messages(user_id):
    sql = "SELECT DISTINCT U.username, M.content, S.message_id, S.content, S.sent_at, S.id FROM messages M, users U, submessages S " \
        "WHERE S.message_id=M.id AND U.id=S.user_id AND U.id=:user_id ORDER BY S.sent_at DESC"
